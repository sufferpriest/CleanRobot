...
//xiaoguo
int8_t cleanrobot_pitch_middle_init_offset = 0;
int8_t cleanrobot_roll_middle_init_offset = 0;

int8_t cleanrobot_auto_path_mode = 0;
int8_t cleanrobot_current_state = 0;
int8_t cleanrobot_run_count = 0;
int8_t cleanrobot_current_turn = 0; //0: turn right, 1: turn left

int16_t cleanrobot_init_head = 0;
int16_t cleanrobot_last_head = 0;
int8_t cleanrobot_head_pid_p = 0;
...
void mixTable() {
  ...
  #elif defined(CLEANER_ROBOT) //xiaoguo
    motor[0] = rcCommand[THROTTLE];
    if (rcCommand[PITCH] > 30) {
		motor[3] = 0;
        motor[1] = motor[2] = MINTHROTTLE / 2 + rcCommand[PITCH] * 2;
        motor[1] -= (150 + cleanrobot_pitch_middle_init_offset * 80);
		if (rcCommand[ROLL] > 30) {
		    motor[1] -= rcCommand[ROLL]*2;
		} else if (rcCommand[ROLL] < -30){
		    motor[2] -= (-rcCommand[ROLL])*2;
		}
    } else if (rcCommand[PITCH] < -30) {
		motor[3] = MAXTHROTTLE;
        motor[1] = motor[2] = MAXTHROTTLE - MINTHROTTLE / 2 + rcCommand[PITCH] * 2;
        motor[1] += (150 + cleanrobot_pitch_middle_init_offset * 80);
	    motor[1] -= cleanrobot_roll_middle_init_offset * 80;
		if (rcCommand[ROLL] > 30) {
		    motor[2] += rcCommand[ROLL]*2;
		} else if (rcCommand[ROLL] < -30){
		    motor[1] += (-rcCommand[ROLL]*2);
        ...
        motor[1] = motor[2] = motor[3] = 0;
		if (!f.ARMED) {
			cleanrobot_pitch_middle_init_offset = rcCommand[PITCH] - 1470;
			cleanrobot_roll_middle_init_offset = rcCommand[ROLL] - 1470;
		}
		if (!f.ANGLE_MODE) {
			cleanrobot_last_head = cleanrobot_init_head = att.heading;
			cleanrobot_head_pid_p = 0;
		}
		if (f.ARMED && f.ANGLE_MODE) {
			if (proximityDis[0] != 1 && cleanrobot_current_state == 0) {
			    motor[3] = 0;
			    motor[1] = motor[2] = MINTHROTTLE / 2 + 200;
		        motor[1] -= (150 + cleanrobot_pitch_middle_init_offset * 80);
				motor[1] -= axisPID[YAW];
				motor[2] += axisPID[YAW];
				/*int8_t head_offset = att.heading - cleanrobot_last_head;
				if (head_offset > 3) {
					//offset to right
					motor[2] -= (250 + head_offset * 2 + cleanrobot_head_pid_p);
					cleanrobot_head_pid_p += 50;
				} else if (head_offset < -3) {
					//offset to left
					motor[1] -= (100 + head_offset * 2 + cleanrobot_head_pid_p);
					cleanrobot_head_pid_p += 50;
				} else {
				    cleanrobot_head_pid_p -= 50;
					if (cleanrobot_head_pid_p < 0) cleanrobot_head_pid_p = 0;
				}*/
			} else {

			    switch(cleanrobot_current_state) {
					case 0:
						cleanrobot_current_state = 1;
						cleanrobot_run_count = 0;
					case 1:
						//first run backward a little
						if (++cleanrobot_run_count < 35) {
							motor[3] = MAXTHROTTLE;
							motor[1] = motor[2] = MAXTHROTTLE - MINTHROTTLE / 2 - 300;
							motor[1] += (150 + cleanrobot_pitch_middle_init_offset * 80);
							motor[1] -= cleanrobot_roll_middle_init_offset * 80;
							break;
						}
						//then turn back ( by turn right first), this is LINE(gong) type algorithm
					    cleanrobot_current_state = 2;
						cleanrobot_run_count = 0;
					case 2:
						if (++cleanrobot_run_count < 100) {
							//turn right first
							motor[3] = 0;
							if (cleanrobot_current_turn == 0) {
							    motor[2] = MINTHROTTLE / 2 + 500;
							    motor[1] = 0;
							} else {
							    motor[1] = MINTHROTTLE / 2 + 500;
								motor[2] = 0;
							}
							break;
						}
						//then
						cleanrobot_current_turn = cleanrobot_current_turn==0?1:0;
						cleanrobot_current_state = 0;
						cleanrobot_run_count = 0;
						break;
				}
			}
