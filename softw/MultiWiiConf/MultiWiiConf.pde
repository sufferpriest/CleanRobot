...
//CLEANERROBOT START xiaoguo
int  proximityDis[] = new int[8];

int xProximityDis = 853; int yProximityDis = 438+tabHeight;
//CLEANERROBOT END xiaoguo
...
private static final int
  ...
  //xiaoguo
  MSP_CLEANER_ROBOT_SENSOR   =121,   //out message         kinds of sensor value
...
public void evaluateCommand(byte cmd, int dataSize) {
  ...
  switch(icmd) {
   ...
   //******************************************************************************************
    ...
    //xiaoguo
    case MSP_CLEANER_ROBOT_SENSOR:
        for(i=0;i<8;i++) {
             proximityDis[i] = read8();
        }
        break;
    default:
...
void draw() {
  ...
  if (init_com==1 && graph_on==1) {
    ...
    if (!toggleRead && !toggleWrite && !toggleSetSetting ) {
      ...
      if ((time-time6)>200 ) {
        time6=time;
        int[] requests = { MSP_RAW_GPS, MSP_COMP_GPS, MSP_ANALOG, MSP_CLEANER_ROBOT_SENSOR};//xiaoguo
        sendRequestMSP(requestMSP(requests));
...
//CLEANERROBOT START 
textFont(font15);
text("Proximity: ",xProximityDis - 200,yProximityDis + 70);
text(proximityDis[0],xProximityDis - 100,yProximityDis + 70);
