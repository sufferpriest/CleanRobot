...
//xiaoguo
#if defined(INFRA_ISR)
#if !defined(INFRA_ISR_MAX_SENSORS) 
  #define INFRA_ISR_MAX_SENSORS    4        // maximum number of sensors we'll allow (can go up to 8)
#endif
#if !defined(INFRA_ISR_SENSITY) 
  #define INFRA_ISR_SENSITY    2
#endif

int infra_isr_pin[INFRA_ISR_MAX_SENSORS]={A0, A1, A2, A3};

static struct {
  uint8_t  proximity[INFRA_ISR_MAX_SENSORS];
  uint32_t  duration[INFRA_ISR_MAX_SENSORS];
} infra_isr4_rx;

void Infra_ISR_init() {
  memset(&infra_isr4_rx, 0, sizeof(infra_isr4_rx));
}

void Infra_ISR_update() {
  for (int i=0; i < INFRA_ISR_MAX_SENSORS; i++) {
    int v1 = analogRead(infra_isr_pin[i]);
    uint8_t now_prx = 0;
	proximityDis[i] = 0;
    if (v1 > 1000) {
	  now_prx = 1;
	  infra_isr4_rx.duration[i] = 0;
	} else if (v1 > 10) {
	  now_prx = 2;
	  infra_isr4_rx.duration[i]++;
	  if (infra_isr4_rx.duration[i] > INFRA_ISR_SENSITY) proximityDis[i] = 1;
	} else {
	  infra_isr4_rx.duration[i] = 0;
	}
	infra_isr4_rx.proximity[i] = now_prx;
  }
}
#else
inline void Infra_ISR_init() {}
void Infra_ISR_update() {}
#endif
